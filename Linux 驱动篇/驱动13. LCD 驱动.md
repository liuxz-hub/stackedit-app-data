...
&emsp;&emsp;`Framebuffer` 在内核中的表现就是 `fb_info` 的结构体， `fb_info` 结构体包含了 `Framebuffer` 设备的完整属性和操作集合，因此每一个 `Framebuffer` 设备都必须有一个 `fb_info`。屏幕驱动
...
&emsp;&emsp;
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTI4NjQ5NjcyLC03NzM1NjM4MywtMjk1Mz
Q0NTMxLDE4OTAxOTY5NTUsMjIxNDU1MDYwLDgxOTQwNzQ2N119
