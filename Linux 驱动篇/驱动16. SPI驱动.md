...
1) 、`spi_transfer` 结构体，此结构体用于描述 SPI 传输信息

2) 、使用`spi_message`之前需要通过spi_message_init函数对其进行初始化，再通过`spi_message_add_tail`函数将 spi_transfer 添加到 spi_message 队列中。
```cpp
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNTQ5OTgyNTI2LC0zMzU1MDg1NzUsMTYzMj
UwNzUwNCwyMTExNjMxMzU1LC0yMDYzNjg5NzAwLDEwNDQxNzE4
NTQsLTIwNzQzNzk0NywtMTcwOTcyMjAxMywtMjAwNzU2NzkyOV
19
-->