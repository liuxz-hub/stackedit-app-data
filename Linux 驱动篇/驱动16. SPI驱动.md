...
程序如下：
```cpp
	/* spi->dev.of_node是子节点的nd
	 * of_get_parent获取对应父节点
	 * icm20608dev.nd是父节点
	 */
	...
	if(icm20608dev.cs_gpio < 0) {
		printk("can't get cd-gpio\r\n");
	}
	ret = gpio_request(icm20608dev.cs_gpio, "cs");
	if(ret < 0){
		printk("failed for gpio request\r\n");
	}
...
{
	u8 value = 1;
	icm20608_write_reg(dev, ICM20_PWR_MGMT_1, 0x80);  	/*复位，睡眠模式*/
	mdelay(50);
...
```cpp
/*spi读取寄存器*/
static int icm20608_read_regs(struct icm20608_dev *dev, u8 reg, u8 *val, int len){
	u8 data = 0;
...
static int icm20608_write_regs(struct icm20608_dev *dev, u8 reg, u8 *buf, int len){
	...
	data = reg & ~0x80;
	spi_write(spi, &data, 1);  	 /*发送要写入的寄存器地址*/
	spi_write(spi, buf, len);	 /*发送数据*/
...
static void icm20608_write_reg(struct icm20608_dev *dev, u8 reg, u8 value){
	u8 buf = value;    //防止被改写，保护措施
	icm20608_write_regs(dev, reg, &buf, 1);
}
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwMTAyMTA4NzgsLTI4Nzk0MzkyMiw2MD
QzNjY5LC03MjMzMzY5NDYsNTQ5OTgyNTI2LC0zMzU1MDg1NzUs
MTYzMjUwNzUwNCwyMTExNjMxMzU1LC0yMDYzNjg5NzAwLDEwND
QxNzE4NTQsLTIwNzQzNzk0NywtMTcwOTcyMjAxMywtMjAwNzU2
NzkyOV19
-->