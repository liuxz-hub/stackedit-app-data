...
&emsp;&emsp;type 为事件类型，查看示例代码 58.1.2.3 可知， EV_KEY 事件值为 1， EV_SYN 事件值为0。因此第 1 行表示 EV_KEY 事件，第 2 行表示 EV_SYN 事件。 code 为事件编码，也就是按键号，查看示例代码 58.1.2.4 可以， KEY_0 这个按键编号为 11，对应的十六进制为 0xb，因此第1 行表示 KEY_0 这个按键事件，最后的 value 就是按键值，为 1 表示按下，为 0 的话表示松开。  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1MTM2MzU2OTgsLTExNDQ1MTg4MywtNT
AxODQwNTc0XX0=
-->