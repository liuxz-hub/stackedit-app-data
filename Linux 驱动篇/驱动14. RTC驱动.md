...
# 2.2 
在驱动与设备匹配之后，`snvs_rtc_probe` 函数就会执行，其中包括申请和初始化 RTC、中断闹钟（中断处理函数是 `snvs_rtc_irq_handler`），以及调用 `devm_rtc_device_register` 函数向系统注册 `rtc_devcie`。
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTY2MDQxMDYwLC02NzI2Mjc1MTUsMTQ5Mj
