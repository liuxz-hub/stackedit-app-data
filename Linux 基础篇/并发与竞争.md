...
3、被自旋锁保护的临界区一定不能调用任何能够引起睡眠和阻塞的API 函数，否则的话会可能会导致死锁现象的发生
4、被自旋锁保护的临界区一定不能调用任何能够引起睡眠和阻塞的 API 函数，否则的话会可能会导致死锁现象的发生
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTI1Mjg4Mjk0MSwyNzkzMzUyNTksMTQwOD
k2NDQsLTQzODY0ODUzOSwtNjk1NTUzODg3LC02OTA0Nzk0MCwx
MTQ0MjQwNzQ4XX0=
-->